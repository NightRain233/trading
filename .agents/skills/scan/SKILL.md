...
5. For each displayed symbol, use compact format:
   ...
   - **反转前兆预警**（提前于翻多信号，捕捉V型反转候选）:
     ...
     - **⚠️ 量化狩猎免疫纪律（必须遵守）**: 看到"放量打穿明牌支撑（20日低点/前低/整数关口）后快速拉回"——第一反应不是抄底，而是"量化在狩猎，躲开"。真正的反转是缩量磨底后自然企稳，不是放量假破位后的V型。遇放量破位: 管住手，等价格回到支撑上方站住2天再谈。系统的防狩猎设计: ①反转前兆要求缩量企稳（放量打穿被过滤）②止损用1.5×ATR动态位（不设明牌固定价位）③分批1/3试探（无肉可猎）④决策锚定周月线（日线假破位是噪音）⑤信号基于收盘价（盘中假动作不算）
   - **牛熊模式切换**（最重要，放在分析最前面）:
     - **必须分市场独立判定！每个市场用自己代表指数的月线中轨方向判定，禁止用一个市场的信号给另一个市场定性。**
     - 各市场代表指数: 🇨🇳A股=上证000001.SS+沪深300 | 🇺🇸美股=SPY+QQQ | ₿加密=BTC-USD+ETH | 🥇黄金=GC=F+518880 | 🏛️债券=511010+TLT
     - 每个市场分别判定:
       - 🟢 **找买点模式** — 该市场代表指数月线中轨↑或走平: 该市场可正常讨论买入
       - 🔴 **保命模式** — 该市场月线中轨↘下降: 该市场停止买入，逢反弹减仓
       - 🚨 **无条件离场** — 该市场股债双杀（股市跌+债券也跌）: 流动性危机信号，清仓离场
     - 判定方法: 每个市场代表指数的月线（timeframe=1M）最后一根boll_mid和4根前的boll_mid比较方向
     - **血泪教训**: 不同市场周期独立（相关性≠1）。7月底A股月线中轨↘（🔴），但美股SPY月线中轨走平（🟢）——若用A股信号给美股定性，会错过美股V型反弹。每个市场用自己天气，别互相撑伞
     - **市场联动仅作辅助**: 美股→A股→BTC有传导（美股强则A股易跟涨），黄金受美元/利率反向影响；联动用于交叉验证，不作为主判依据
     - 规则: "等回踩/等突破"的右侧逻辑只允许在该市场🟢模式下使用；该市场🔴模式下禁止用回踩逻辑接飞刀
